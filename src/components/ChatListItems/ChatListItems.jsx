 ...
 */
export const ChatListItems =({chats, handleSwitchPage}) => {
    //return chats.map((chat, id) => <ChatListItem {...{chat}} key={id} />)
    return Object.keys(chats).map((chatName, id) => <ChatListItem {...{chat: chats[chatName]}} handleSwitchPage={handleSwitchPage} key={id} />);
};
